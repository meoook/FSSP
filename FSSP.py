...
def get_req(task_uuid, status='result'):
    ...
    to_log('Task_UUID failure: ' + str(task_uuid), 2)
    return False  # TO LOG
            ...
            if x == y:
                ...
                if isinstance(ar[y], str):
                    ...
                    if typo == 2 and ar[x][0] == ar[y][0] and ar[x][1] == ar[y][1]:
                        ...
                        to_log('Check elem error. Type of element: ' + str(typo), 2)  # TO LOG
                        to_del.append(y)
    ...
    if LOG_TO_FILE:  # Если логирование включено
        ...
        if not os.path.isfile(LOG_FILE_NAME):
            with open(LOG_FILE_NAME, "w") as filo:
                filo.write(
                    time.strftime("%d.%m.%y %H:%M:%S", time.gmtime()) + ': Created file log ' + LOG_FILE_NAME + '\n')
